...
export default function CreateInterviewQuestions(props: CreateInterviewQuestionsProps) {
  ...
  const canSave = !isSaving && fields.length >= questionCount && formState.isValid;

  return (
    <div className="flex w-full flex-col gap-4 px-3 py-1">
      <Button variant="outline" type="button" size="sm" className="w-fit" onClick={onBack}>
        <ChevronLeft size={20} />
        <span>Back</span>
      </Button>
      ...
      <div className="flex justify-end">
        <Button type="submit" disabled={!canSave} className="bg-primary hover:bg-primary/90">
